...
def plot_hcan(spat,rname_all,plot_path, points):
    #points = [27906, 39222, 44444, 48718]
    pegnames_sim = ["KT","KH","KP","MS","HD","RB","KB","HB"]
    pegnames_obs_duenn= ['KT_duenn','PK_duenn']
    pegnames_obs_dicht=['KT_dicht','AR_dicht','PK_dicht','RT_dicht']
    ...
    name     = plot_path + 'hcan_all_obs_' + rname + '.png'

    hcan_month = np.zeros(12)
    #save monthly hcan
    hdf5file = '/work/gg0877/g260204/data/pickle_files/' +rname + 'hcan_month.hdf5'
    f = h5py.File(hdf5file,"w")

    hcan = read_data('ICM_hcansav',files_sorted,face=points)
    for p in range(0,len(points)):
        tmp = hcan[:,p]
        for i in range(1,13):
            hcan_month[i-1] = np.mean(tmp[np.where(np.array(months)==i)])
        f.create_dataset("hcan_"+pegnames_sim[p],data=hcan_month,dtype='f')
        plt.plot(range(1,13),hcan_month*100,label=pegnames_sim[p])
    f.close()
