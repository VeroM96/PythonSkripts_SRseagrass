...
#####################################################################
    for year in range(2010,2016):
        ...
        files_sorted = sort_files(in_path+run+'/outputs')
        if year==2010:
            hcan = read_data('ICM_hcansav',files_sorted,face=iin)
            hcan[:,np.max(hcan,0)==0]=np.nan
        else:
            tmp = read_data('ICM_hcansav',files_sorted,face=iin)
            tmp[:,np.max(tmp,0)==0]=np.nan
            hcan = np.append(hcan,tmp,0)
        print(np.shape(hcan))
    ...
    hcan_all = []
    for m in month:
        hcan_m = hcan[np.array(months)==m,:]
        hcan_m = hcan_m.flatten()*100
        hcan_m = hcan_m[~np.isnan(hcan_m)]
        hcan_all.append(hcan_m)
    ...
    for m in months:
        ...
        for d in range(0,2):
            ...
            obs_mean[m-1,d] = tmpd['Blattlänge Zostera noltei in cm'].mean()
            obs_min[m-1,d] = tmpd['Blattlänge Zostera noltei in cm'].min()
            obs_max[m-1,d] = tmpd['Blattlänge Zostera noltei in cm'].max()

    month=range(1,13)

    fig = plt.figure(figsize=(4,4))

    plt.boxplot(hcan_all,0,'') 

    #plot obs:
    plt.fill_between(month, obs_max[:,1], obs_min[:,1],alpha=0.2,color='cadetblue',zorder=0)
    plt.plot(month,obs_mean[:,1],color='cadetblue',zorder=1,linewidth=1,linestyle='--')
    plt.fill_between(month, obs_max[:,0], obs_min[:,0],alpha=0.2,color='darkseagreen',zorder=0)
    plt.plot(month,obs_mean[:,0],color='darkseagreen',zorder=1,linewidth=1,linestyle='--')

    plt.ylabel('canopy height [cm]')
    plt.xlabel('month of the year')
    plt.title(meadow)
    plt.tight_layout()

    plt.tight_layout()
    plt.savefig(plotpath + 'hcan_simobs_'+meadow+'.png')
