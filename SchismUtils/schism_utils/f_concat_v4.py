...
#   tformat  : format for time output (default: same as in netcdf)
#               -num2date: output as cftime._cftime.DatetimeGregorian -> needs additional modules 
#                          for plotting (cftime,munits)
#output:
...
def read_data(varname, ncf_all,*,node="",vlayer="",face="",ntime="",tformat=""):
    ...
    if 'time' in varname:
        if tformat=='num2date':
            ...
            vardata[varname.index('time')] = num2date(vardata[varname.index('time')],units = time_units)
    
    #remove unnecessary array if only one variable is read
    if len(varname)==1:
        vardata= vardata[0]
